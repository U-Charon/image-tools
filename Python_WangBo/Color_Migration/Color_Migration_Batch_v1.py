...
def global_adw(img_dir, p=0.1, overlap=0.2):
    ...
    imgs_Sta = np.zeros(shape=(len(origin_img_paths),
                               3))  # "3"分别对应：mean var pix_num
    ...
    imgs_mean = np.sum(imgs_Sta[:, 0] * imgs_Sta[:, 2]) / np.sum(imgs_Sta[:,
                                                                          2])
...
def local_mean_function(adw_size, adw_stride, img_path):
    ...
    # cv2.imshow(f'img_{img_row, img_col}', img_data)
    # cv2.waitKey()
    ...
    img_top_path = img_position[0][:-1] + str(img_row-1) + "__" + str(img_col) 
    ...
    # cv2.imshow(f'img_padding_{img_row, img_col }', img_padding)
    # cv2.waitKey()
    ...
    for m in range(num_h):
        ...
        for n in range(num_w):
            ...
            # cv2.waitKey()
            # temp_path = f'F:\data_analysis\Multiple_Auto-Adapting_Color_Balancing\Bantch_Color_Migration\B{m, n}.tif'
            ...
            # print(local_mean_map[m, n])
    # print('local_mean_map', local_mean_map.shape)
    ...
    # cv2.imshow('mm', mean_map)
    # cv2.waitKey()

    return mean_map


def batch_gamma_correction(source_dir, target_dir, output_dir, alpha=1, p=0.1, overlap=0.2):
    """
    Parameters
    ----------
    ...
    source_adw_size, source_adw_stride = global_adw(source_dir, p, overlap)

    target_dir = Path(target_dir)
    target_adw_size,  target_adw_stride  = global_adw(target_dir, p, overlap)
    ...
    for source_path, target_path in zip(source_img_paths, target_img_paths):
        ...
        s_mean_map = local_mean_function(source_adw_size, source_adw_stride, source_path)
        t_mean_map = local_mean_function(target_adw_size, target_adw_stride, target_path)
...
if __name__ == '__main__':
    ...
    # source_dir = r"F:\data_analysis\Multiple_Auto-Adapting_Color_Balancing\Bantch_Color_Migration\source"
    # target_dir = r"F:\data_analysis\Multiple_Auto-Adapting_Color_Balancing\Bantch_Color_Migration\target"
    # output_dir = r"F:\data_analysis\Multiple_Auto-Adapting_Color_Balancing\Bantch_Color_Migration\output"

    # batch_gamma_correction(source_dir, target_dir, output_dir, alpha=1, p=0.1, overlap=0.2)
    ...
    source_dir = r"F:\data_analysis\Color_Migration\Color_Migration_bantch\normal\s_img"
    reference_dir = r"F:\data_analysis\Color_Migration\Color_Migration_bantch\normal\r_img"
    output_dir = r"F:\data_analysis\Multiple_Auto-Adapting_Color_Balancing\Bantch_Color_Migration\big_out"
