...
class DataDistributionTransform:
    ...
    def __dataStatistics(self, img_data, c_color):
        """
        ...
        histogram = np.zeros(shape=(len(c_color), 256, 1))
        for ch, col in enumerate(c_color):
    ...
    def dataset_distribution(self, dataset_dir, picture=True, c_color=None):
        """
        dataset_dir: 图像数据文件的夹路径
        ...
        for i, img_dir in enumerate(img_name_list):
            ...
            # img_data = cv2.imread(os.path.join(self.dataset_dir, img_dir))  # BGR
            img_data = tiff.imread(os.path.join(dataset_dir, img_dir))  # [RGB]
            histogram = histogram + self.__dataStatistics(img_data, c_color)  # todo 调试
            t2 = time.time()
        ...
        if picture:
            ...
            plt.title(f"{dataset_name}'s Pixel Statistical Histogram", fontsize=36)
            plt.xlabel('pixle value', fontsize=36)
            plt.ylabel('pixle frequency', fontsize=36)
            plt.xlim([0, 256])
            width_val = (1 - 0.1) / len(c_color)
            xticks = np.arange(256)
            for c in range(len(c_color)):
                plt.bar(xticks + width_val * c, histogram[c], alpha=0.6, width=width_val, color=c_color[c],
                        label=c_color[c])
    ...
    def __transfToUniform(origin_data, cdf):
        ...
        uniform_data = np.array(np.zeros_like(origin_data), dtype=cdf.dtype)
        for i in range(h):
            for j in range(w):
                for k in range(c):
    ...
    def __transfToNormal(self, uniform_data):
        ...
        # cv2.normalize(src=Z1, dst=Z1, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        # cv2.normalize(src=Z2, dst=Z2, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        Z3, Z4 = self.__boxMullerTransform(uniform_data[..., 1:2], uniform_data[..., 2:3])
        # cv2.normalize(src=Z3, dst=Z3, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        # cv2.normalize(src=Z4, dst=Z4, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        Z5, Z6 = self.__boxMullerTransform(uniform_data[..., 2:3], uniform_data[..., 0:1])
        # cv2.normalize(src=Z5, dst=Z5, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        # cv2.normalize(src=Z6, dst=Z6, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        normal_data = np.concatenate((Z1, Z2, Z3, Z4, Z5, Z6), axis=2)
        # normal_data = np.array(normal_data, dtype='uint8')
        return normal_data
    ...
    def originTransfToStdNormal(self, dataset_dir):
        ...
        for i, name in enumerate(uniform_name_list):
            ...
            cv2.normalize(src=normal_data, dst=normal_data, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
            normal_data = np.array(normal_data, dtype='uint8')

            # cv2.imshow('012', normal_data[..., [0, 1, 2]])
            # cv2.imshow('345', normal_data[..., [0, 2, 4]])
            # print(np.min(normal_data[..., 0]), np.max(normal_data[..., 0]))
            # print(np.min(normal_data[..., 1]), np.max(normal_data[..., 1]))
            # print(np.min(normal_data[..., 2]), np.max(normal_data[..., 2]))
            # print(np.min(normal_data[..., 3]), np.max(normal_data[..., 3]))
            # print(np.min(normal_data[..., 4]), np.max(normal_data[..., 4]))
            # print(np.min(normal_data[..., 5]), np.max(normal_data[..., 5]))
            # cv2.waitKey()
    ...
    def origin1transfToOrigin2(self, dataset_dir1, dataset_dir2):
        ...
        for i, name in enumerate(img_name_list):
            ...
            new_save_name = os.path.join(new_save_dir, name)
            tiff.imwrite(new_save_name, data=img_new)
...
if __name__ == '__main__':
    ...
    dataset_dir01 = r'F:\data_distributed_transform\itamiH29\Images_to_Normals'  # itami城
    hist = data_distribute_transf.dataset_distribution(dataset_dir01, picture=True,
                                                       c_color=['red', 'green', 'blue', 'yellow', 'pink',
                                                                'orange'])  # , 'blue', 'yellow', 'pink', 'orange'
    print('像素比例分布表', hist)

    # # 2. 将 data_dir 转换为 均匀分布数据
    # dataset_dir01 = r'F:\data_distributed_transform\itamiH29\Images'  # itami城
    # data_distribute_transf.originTransfToUniform(dataset_dir01, use_uniform01=False)
    ...
    # dataset_dir01 = r'F:\data_distributed_transform\itamiH29\Images'  # itami城
    # dataset_dir02 = r'F:\data_distributed_transform\ancheng\h29'  # 安城
    # data_distribute_transf.origin1transfToOrigin2(dataset_dir01, dataset_dir02)

    # # 分通道 查看 normal_data 的数据 的成像效果
    # img_path= r'F:\data_distributed_transform\itamiH29\Images_to_Normals\I001.tif'
    # img_data = tiff.imread(img_path)
    # print(img_data.shape)
    # cv2.imshow('0', img_data[..., :3])
    # cv2.imshow("1", img_data[..., 3:])
    # cv2.waitKey()
